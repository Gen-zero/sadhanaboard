...
export const adminApi = {
  ...
  connectUserStream(onUserUpdate: (user: any) => void, onError?: (err: unknown) => void) {
    try {
      if (this._socket) {
        this._socket.disconnect();
        this._socket = null;
      ...
      socket.on('connect', () => {
        try { 
          ...
          console.error('Failed to subscribe to users room', e);
        }
      });
  ...
  // Library management real-time streaming
  connectLibraryStream(onBookUpdate: (book: any) => void, onError?: (err: unknown) => void) {
    try {
      if (this._socket) {
        this._socket.disconnect();
        this._socket = null;
      ...
      socket.on('connect', () => {
        try { 
          ...
          console.error('Failed to subscribe to library room', e);
        }
      });
  ...
  async listWaitlist(q = '', limit = 20, offset = 0, status = 'all') {
    const params = new URLSearchParams({ q, limit: String(limit), offset: String(offset), status });
    return adminRequest<{ waitlist: Array<{ id: string; name: string; email: string; reason: string; status: string; created_at: string; updated_at: string }>, total: number, limit: number, offset: number }>(`/waitlist?${params.toString()}`);
  },
  
  async updateWaitlistEntry(id: string, payload: { status: string }) {
    return adminRequest<{ message: string; entry: any }>(`/waitlist/${id}`, { method: 'PATCH', body: JSON.stringify(payload) });
  },
