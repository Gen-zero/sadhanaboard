              ...
              <div className="relative z-10 p-4 sm:p-6">
                ...
                <div className="mt-2">
                  <div className="space-y-4 sm:space-y-5">
                    ...
                    {/* Sadhana Start/End Dates Section */}
                    {selectedDay.sadhanaData?.sadhanaDetails && selectedDay.sadhanaData.sadhanaDetails.length > 0 && (
                      <div className="bg-gradient-to-r from-purple-900/20 to-black/20 p-3 sm:p-5 rounded-lg sm:rounded-xl border border-purple-800/30 shadow-lg relative overflow-hidden">
                        {/* Sacred Circuit Pattern Overlay */}
                        <div className="absolute inset-0 opacity-5 pointer-events-none">
                          <SacredCircuitPattern color="#DC143C" />
                        </div>
                        ...
                        <div className="relative z-10">
                          <h4 className="font-bold text-purple-400 mb-2 sm:mb-3 text-base sm:text-lg flex items-center gap-1 sm:gap-2">
                            <div className="w-3 h-3 sm:w-4 sm:h-4 rounded-full bg-purple-500 shadow-lg"></div>
                            Sadhana Papers
                          </h4>
                          <div className="space-y-2">
                            {selectedDay.sadhanaData.sadhanaDetails.map((sadhana, index) => (
                              <div key={index} className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-black/30 rounded-lg sm:rounded-lg border border-purple-900/30 hover:bg-purple-900/20 transition-all duration-300 relative overflow-hidden">
                                {/* Sacred Circuit Pattern Overlay */}
                                <div className="absolute inset-0 opacity-5 pointer-events-none">
                                  <SacredCircuitPattern color="#DC143C" />
                                </div>
                                ...
                                <div className="relative z-10 flex flex-col w-full">
                                  <div className="flex items-center gap-2 sm:gap-3">
                                    <div className="w-2 h-2 sm:w-3 sm:h-3 rounded-full bg-purple-500 shadow-sm"></div>
                                    <span className="text-gray-200 font-medium text-sm sm:text-base">{sadhana.name}</span>
                                  </div>
                                  <div className="ml-5 mt-1 text-xs sm:text-sm text-gray-400">
                                    <span className="inline-block mr-2">
                                      {sadhana.type === 'start' ? 'Starts' : 'Ends'}:
                                      {new Date(sadhana.startDate).toLocaleDateString()}
                                    </span>
                                    <span className="inline-block">
                                      Ends: {new Date(sadhana.endDate).toLocaleDateString()}
                                    </span>
                                  </div>
                                </div>
                              </div>
