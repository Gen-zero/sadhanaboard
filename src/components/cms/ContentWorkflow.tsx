...
export default function ContentWorkflow() {
  ...
  const [items, setItems] = useState<CmsTemplate[]>(templates || []);
  const [statusMap, setStatusMap] = useState<{ [k: string]: CmsTemplate[] }>({ draft: [], in_review: [], approved: [] });
  ...
  const moveItem = (item: CmsTemplate, to: string) => {
    // local move; real backend publish flow would be implemented later
    setStatusMap(s => {
      const copy: { [k: string]: CmsTemplate[] } = { draft: [...s.draft], in_review: [...s.in_review], approved: [...s.approved] };
      // remove from any lane
      for (const k of Object.keys(copy)) copy[k] = copy[k].filter((x) => x.id !== item.id);
