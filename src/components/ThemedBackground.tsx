        ...
        case 'neon':
          ...
          for (let i = 0; i < particleCount; i++) {
            particles.push({
              ...
              alpha: Math.random() * 0.6 + 0.2,
              pulseDirection: Math.random() > 0.5 ? 1 : -1,
              color: Math.random() > 0.6 ? 'rgba(0, 255, 255, 0.8)' : Math.random() > 0.5 ? 'rgba(255, 0, 255, 0.7)' : 'rgba(0, 255, 127, 0.6)',
          ...
          // Create neon cyber tech spiritual elements
          for (let i = 0; i < spiritualElementCount; i++) {
            const elementType = Math.floor(Math.random() * 4); // 0-3 for different neon elements
            
            spiritualElements.push({
              x: Math.random() * canvas.width,
              y: Math.random() * canvas.height,
              size: Math.random() * 50 + 30,
              speedX: (Math.random() * 0.6 - 0.3) * 0.7,
              speedY: (Math.random() * 0.6 - 0.3) * 0.7,
              rotation: Math.random() * Math.PI * 2,
              rotationSpeed: (Math.random() * 0.01 - 0.005) * 1.5,
              alpha: Math.random() * 0.5 + 0.3,
              pulseDirection: Math.random() > 0.5 ? 1 : -1,
              type: elementType,
              ...
              update: function() {
                ...
                this.alpha += 0.004 * this.pulseDirection;
                if (this.alpha <= 0.3 || this.alpha >= 0.8) {
                  this.pulseDirection *= -1;
                }
              ...
              draw: function() {
                ...
                switch (this.type) {
                  case 0: // Neon grid
                    drawNeonGrid(ctx, this.x, this.y, this.size * 0.6, this.alpha);
                    break;
                  case 1: // Neon circuit
                    drawNeonCircuit(ctx, this.x, this.y, this.size * 0.5, this.alpha);
                    break;
                  case 2: // Neon hologram
                    drawNeonHolo(ctx, this.x, this.y, this.size * 0.4, this.rotation, this.alpha);
                    break;
                  case 3: // Neon data stream
                    drawNeonDataStream(ctx, this.x, this.y, this.size * 0.3, this.alpha);
                    break;
                }
              }
            });
