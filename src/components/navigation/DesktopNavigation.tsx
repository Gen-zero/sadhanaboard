...
const DesktopNavigation: React.FC<DesktopNavigationProps> = ({
  ...
  return (
    <TransparentGlassMorphismContainer
      className={`fixed left-0 top-0 h-screen z-40 sidebar-seamless flex flex-col overflow-hidden border-r border-red-900/30 ${!isSidebarOpen ? 'pointer-events-none' : ''
        }`}
    >
      <div
      ...
      >
        ...
        {/* Removed bottom-left and bottom-right brackets to declutter footer area */}
        ...
        <div className="flex flex-col items-center justify-center p-6 space-y-3 border-b border-red-900/30 flex-shrink-0 relative z-10 bg-black/20 backdrop-blur-sm rounded-lg m-2">
          ...
          >
            <div className="deity-glow-container hover:animate-float-gentle overflow-visible relative w-24 h-24 flex items-center justify-center">
          ...
          <>
            <div className="text-center">
              <h3 className="text-xl font-medium text-white">{currentThemeData.name}</h3>
              <p className="text-sm text-yellow-500/80 mantra-gradient-text font-semibold">{currentThemeData.element} Element</p>
            </div>
            <div className="w-full h-px bg-gradient-to-r from-transparent via-red-600/50 to-transparent"></div>
          </>
        </div>

        <nav className="flex-1 px-3 py-2 overflow-y-auto sidebar-scrollbar nav-scrollable-area relative z-10" role="navigation" aria-label="Main navigation">
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.name}
                  to={item.path}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 nav-item touch-target-large mobile-focus-visible relative overflow-hidden ${active
                ...
                >
                  ...
                  <span className={`flex items-center justify-center w-10 h-10 rounded-lg transition-all duration-300 ${active ? 'bg-red-800/50 text-yellow-400 scale-110 shadow-lg' : 'text-gray-400 bg-black/20'
                    }`}>
                    {item.showLogo ? (
                      <img
                  ...
                  <span className="truncate flex-1 font-medium text-base">{item.name}</span>
        ...
        <div className="mt-auto p-4 border-t border-red-900/30 flex-shrink-0 sidebar-footer relative z-10 bg-black/20 backdrop-blur-sm rounded-xl mx-2 mb-2">
          <div className="flex flex-col space-y-4">
            {user ? (
              <div className="flex flex-col space-y-4">
                ...
                <Link
                  to="/profile"
                  className={`flex items-center rounded-xl transition-all duration-300 px-4 py-3 touch-target-large mobile-focus-visible relative overflow-hidden border ${isActive('/profile')
                ...
                >
                  ...
                  <span className="font-medium flex-1 text-base">My Profile</span>
                ...
                >
                  ...
                  <span className="text-red-300 font-medium group-hover:text-red-200 text-base">Disconnect</span>
                </Button>
              </div>
            ) : (
              <Button
